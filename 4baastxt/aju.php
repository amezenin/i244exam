...
$types = array('txt', 'doc', 'docx', 'odt');
// Максимальный размер файла
$size = 1024000;
$extension = end(explode(".", $_FILES['lae']["name"])); //on berjot rashirenie. tok zna4enie posle to4ki doc jne
...
if ($_POST['lae'])
{
	...
	if (!move_uploaded_file($_FILES['lae']['tmp_name'], $path . $_FILES['lae']['name']))
		echo 'Midagi laks valesti';
		 ...
		 echo 'Fail on laetud <a href="' . $path . $_FILES['lae']['name'] . '">Lae alla faili</a> ' ;
}
	//количество файлов
	if($_POST['nae']){
		$dir = opendir('up/');
